(function () {
  ...
  var setId = params.get('set') || 'set_1';
  var tasks = data.sessions[setId].tasks, index = 0, retries = [];
    ...
    String(fen).split(' ')[0].split('/').forEach(function (rank, r) {
      ...
      rank.split('').forEach(function (symbol) {
        ...
        for (var n = 0; n < count; n += 1) {
          ...
          var coordinate = String.fromCharCode(97 + file) + (8 - r);
          square.setAttribute('aria-label', coordinate);
          if (derivation && coordinate === derivation.from) square.classList.add('origin-square');
          if (derivation && coordinate === derivation.to) square.classList.add('target-square');
  ...
  function render() {
    var task = tasks[index]; board(task.presented_fen, task.answer_derivation || {});
